...
| 지표 | 딥리서치만 사용 | 스마트 라우팅 적용 후 | 개선율 |
|------|---------------|-------------------|--------|
| 평균 응답 시간 | 2-4분 | 5-10초 (일반) | **95% 단축** |
| 쿼리당 API 호출 | 14회 이상 | 2-3회 | **86% 감소** |
| LLM 토큰 사용량 | 고비용 | 필요에 비례 | **87% 감소** |
| 단순 질의 비용 | ~$0.05 | ~$0.003 | **94% 절감** |

**Gatekeeper Router**가 들어오는 질의를 3개 경로로 분류:
1. **일반 채팅** - 간단한 질문은 즉시 응답 (불필요한 검색 없음)
2. **빠른 팩트체크** - 사실 확인 질의는 2-3개 검색 결과 + 간단 검증
3. **딥리서치** - 복합 분석은 전체 11노드 파이프라인 가동

이 라우팅만으로 모든 질의에 딥리서치를 돌리던 것 대비 평균 API 비용 **86% 절감**.
