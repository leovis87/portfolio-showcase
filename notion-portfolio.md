...
### 1. API 비용 최적화 — 3단계 LLM 라우팅

**문제:** 24개 노드가 모두 Gemini Pro를 호출하면 API 비용이 과다하고 응답 속도도 느림.

**접근:** "비싼 모델 = 좋은 결과"라는 가정을 벤치마크로 검증.

**해결 — 3단계 라우팅 체계 구축:**

| 계층 | 모델 | 대상 노드 | 근거 |
|------|------|-----------|------|
| **Tier 1 (무료)** | EXAONE 3.5 7.8B (로컬) | 쿼리 분류, 검색 쿼리 생성, 검색 필요성 판단, 전략 옵션 생성, 후속 질문 생성 (5개) | `benchmark_hybrid.py`로 A/B 테스트 → 5개 노드 **100% 품질 동등** 확인 |
| **Tier 2 (저비용)** | Gemini 2.5 Flash (클라우드) | Writer, Publisher 등 고복잡도 노드 | 메인 워크호스 |
| **Tier 3 (통합)** | ~~Gemini Pro~~ → Flash 전환 | Evaluator, Fact Checker, 관련성 검증 (판단 노드) | `benchmark_nodes.py`로 검증 → Pro 대비 **-4% 품질, 2배 속도** → Pro 유지 비용 대비 이점 없음 |

**구현:** `config.py`의 `HYBRID_NODE_ROUTING` 테이블 + `llm_gateway.py`의 `resolve_is_secure()` 함수가 노드명 기반으로 Local/Cloud를 동적 결정.

**결과:** LOW 복잡도 노드 5개의 클라우드 API 호출 완전 제거 + Pro 의존도 0%로 감소. 품질 저하 없이 비용 대폭 절감.

### 2. 불필요한 기능 제거 — Publisher Rewrite 삭제

**문제:** Publisher 노드에서 Writer가 생성한 보고서를 한 번 더 "리라이트"하는 기능이 있었음. 직관적으로는 "한 번 더 다듬으면 좋아질 것" 같지만, 실제 개선 효과가 있는지 의문.

**검증:** `benchmark_writer.py`로 Rewrite ON/OFF 비교 → **0% 품질 개선** 확인. 오히려 리라이트 과정에서 원본의 참고문헌 링크가 깨지는 부작용 발생.

**결정:** Rewrite 기능 전면 삭제. Publisher는 인포그래픽 생성 + 참고문헌 URL 복원 + 최종 출력 구성에 집중.

**교훈:** "기능 추가"보다 **"불필요한 기능 제거"**가 더 어려운 판단. 벤치마크 데이터가 있어야 확신을 갖고 삭제할 수 있음.

### 3. 동적 HITL 설계 — 자동화와 사용자 제어의 균형

**문제:** Deep Research는 검색→평가→재검색→작성까지 2~4분 소요. 완전 자동화하면 사용자가 원치 않는 방향으로 장시간 진행될 수 있고, 매 단계 확인하면 자동화의 의미가 없음.

**설계 원칙:** "되돌리기 비용이 큰 지점"에서만 사용자 확인.

**구현:**
- **딥리서치 진입 전**: 3가지 전략 옵션(빠른 요약 / 균형 분석 / 심층 조사) 제시 → 사용자 선택
- **진행 중**: SSE 스트리밍으로 실시간 진행률 표시 (어떤 소스를 검색 중인지 투명하게 공개)
- **자동 진행**: 평가→재검색 루프(최대 3회), 코드 자기수정 루프(최대 2회)는 자동

**결과:** 사용자는 "방향"만 결정하고 실행은 시스템이 자율적으로 처리. 중간에 취소해도 `force_mode`로 일반 채팅으로 즉시 전환.

### 4. LLM 코드의 테스트 전략 — Mock + 벤치마크 이원 체계

**문제:** LLM 응답은 비결정적(non-deterministic)이라 기존 단위 테스트만으로는 검증 불가. 하지만 "테스트 불가능"이라고 넘기면 품질 보증이 없음.

**해결 — 2가지 테스트 계층:**

| 계층 | 도구 | 검증 대상 | 실행 시간 |
|------|------|-----------|-----------|
| **단위 테스트** | pytest (83개) | JSON 파싱, URL 정규식, 포맷터, 캐시 CRUD 등 결정론적 로직 | < 1초 |
| **벤치마크** | benchmark_*.py (3종) | Writer 품질, 판단 노드 정확도, 하이브리드 라우팅 품질 비교 | 수 분 |

**핵심:** `conftest.py`에서 30+ 모듈을 Mock 처리하여 LLM 없이도 파이프라인 로직을 검증. 실제 LLM 품질은 벤치마크 스크립트로 별도 측정하여 모델 교체/프롬프트 변경 시 회귀 테스트로 활용.

### 5. 딥리서치 보고서 구조가 질문과 무관한 문제

**문제:** 모든 질문에 대해 고정 템플릿이 적용됨. "전기차 vs 수소차 비교해줘"라고 물어도 "Implementation Guide (코드 예시)" 섹션이 생성됨.
...
### 7. React Native Markdown 라이브러리 레이아웃 파괴
...
모든 모델/프롬프트/기능 변경은 벤치마크 데이터로 검증:
- `benchmark_writer.py`: 보고서 구조, 섹션 수, 참고문헌 형식 등 Writer 출력 품질 정량 평가
- `benchmark_nodes.py`: Evaluator, Fact Checker, 관련성 검증 등 판단 노드 정확도 측정
- `benchmark_hybrid.py`: 로컬 vs 클라우드 A/B 테스트로 하이브리드 라우팅 타당성 검증

"감"이 아닌 **데이터**로 결정하고, 회귀 테스트로 품질을 지속 보증.
